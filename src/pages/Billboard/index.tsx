...
const BillboardMemo = React.memo(
	(props: Props) => {
		const gradient =
		...
		const data = useMemo(
			() =>
				props.mergeResults.filter(
					(e) =>
						e.user.weleEmail !== "weenjoylearningenglish@gmail.com"
				),
			[props.mergeResults]
		);
		return (
					...
					>
						...
						>
							...
							<StyledBillboardContent>
								<StyledHeader>Billboard</StyledHeader>
								<FlatList
									data={data}
	...
	(prev, next) =>
		prev.theme === next.theme && prev.mergeResults === next.mergeResults
);
